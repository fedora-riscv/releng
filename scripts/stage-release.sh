...
sudo -u ftpsync compose-partial-copy --arch=armhfp --arch=x86_64 --arch=src \
     ...
     --variant Everything --variant Cloud --variant Container --variant Modular --variant Server --variant Silverblue --variant Kinoite --variant Spins --variant Workstation \
...
sudo -u ftpsync compose-partial-copy --arch=aarch64 \
     $BASE/$SHORTRELEASEVER/$COMPOSEID/compose/$dir/ $DESTDIR/$RELPREFIX$RELEASEVER/$dir/ \
     --variant Everything --variant Cloud --variant Container --variant Modular --variant Server --variant Silverblue --variant Kinoite --variant Workstation \
     --link-dest=/pub/fedora/linux/development/$SHORTRELEASEVER/Everything/ --link-dest=$STAGEDIR/$STAGE/Everything/ --link-dest=$STAGEDIR/$STAGE/$dir/
...
sudo -u ftpsync compose-partial-copy --arch=i386 --arch=ppc64le --arch=s390x \
     ...
     --variant Everything --variant Cloud --variant Container --variant Labs --variant Modular --variant Server --variant Silverblue --variant Kinoite --variant Spins --variant Workstation \
     --link-dest=/pub/fedora/linux/development/$SHORTRELEASEVER/Everything/ --link-dest=$STAGEDIR/$STAGE/Everything/ --link-dest=$STAGEDIR/$STAGE/$dir/
